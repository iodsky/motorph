...
public class LeaveRequestService {
    ...
    public LeaveRequest getLeaveRequestById(String leaveRequestId) {
        return leaveRequestRepository.findById(leaveRequestId)
                .orElseThrow(() -> new ApiException(HttpStatus.NOT_FOUND, "Leave request " + leaveRequestId + " not found"));
    }
    ...
    public LeaveRequest updateLeaveRequest(String leaveRequestId, LeaveRequestDto dto) {
        ...
        if (!entity.getLeaveStatus().equals(LeaveStatus.PENDING)) {
            throw new ApiException(HttpStatus.BAD_REQUEST, "Cannot delete processed leave request");
        }
    ...
    public LeaveRequest updateLeaveStatus(String leaveRequestId, LeaveStatus newStatus) {
        ...
        if (!leaveRequest.getLeaveStatus().equals(LeaveStatus.PENDING)) {
            throw new ApiException(HttpStatus.BAD_REQUEST, "Leave request " + leaveRequestId + " has already been processed");
        }
        ...
        try {
            ...
            throw new ApiException(HttpStatus.BAD_REQUEST, "Leave credits were modified by another process.");
        }
    }
    ...
    private void validateDates(Long employeeId, LeaveRequestDto dto) {
        ...
        if (isWeekend(startDate)) {
            throw new ApiException(HttpStatus.BAD_REQUEST, "Start date must be a weekday");
        }

        LocalDate endDate = dto.getEndDate();
        if (isWeekend(endDate)) {
            throw  new ApiException(HttpStatus.BAD_REQUEST, "End date must be a weekday");
        }
        ...
        if (leaveRequestRepository.existsByEmployee_IdAndStartDateAndEndDate(
            ...
            throw new ApiException(HttpStatus.CONFLICT, "Duplicate leave request");
        }
        ...
        if (leaveRequestRepository
            ...
            throw new ApiException(HttpStatus.BAD_REQUEST, "Leave request overlaps with an existing pending or approved leave");
        }
    }
    ...
    private LeaveType resolveLeaveType(String leaveTypeStr) {
        ...
        try {
            ...
            throw new ApiException(HttpStatus.BAD_REQUEST, "Invalid leave type: " + leaveTypeStr);
        }
        return type;
    }
